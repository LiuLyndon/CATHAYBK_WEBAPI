...
void ConfigureServices(WebApplicationBuilder builder)
{
    ...
    // builder.Services.AddSingleton<IJwtUtils, JwtTokenUtil>(); // 註冊 JWT 工具類別
    ...
    builder.Services.AddSwaggerGen(options =>
    {
        ...
        {
            In = ParameterLocation.Header, // JWT 放在請求的 Header
            Description = "請輸入 JWT Token，格式為 Bearer <token>",
            Name = "Authorization", // Header 的名稱
            Type = SecuritySchemeType.Http, // 授權類型為 HTTP
            BearerFormat = "JWT", // 指定使用 JWT 格式
            Scheme = "Bearer" // 使用 Bearer 驗證
        });
