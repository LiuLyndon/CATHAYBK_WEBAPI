...
{
    /// <summary>
    /// 自訂的 OpenAPI 運行過濾器，用於根據方法上的 UpdatedResponseTypeAttribute 設置回應的 Schema。
    /// </summary>
    public class UpdatedResponseTypeOperationFilter : IOperationFilter
    {
        /// <summary>
        ...
        {
            // 獲取 UpdatedResponseTypeAttribute 屬性
            var updatedResponseTypeAttribute = context.MethodInfo
                .GetCustomAttributes(typeof(UpdatedResponseTypeAttribute), false)
                .FirstOrDefault() as UpdatedResponseTypeAttribute;

            // 如果找到屬性則繼續處理
            if (updatedResponseTypeAttribute != null)
            {
                // 使用 SchemaGenerator 生成指定 ResponseType 的 Schema
                var schema = context.SchemaGenerator.GenerateSchema(
                    updatedResponseTypeAttribute.ResponseType,
