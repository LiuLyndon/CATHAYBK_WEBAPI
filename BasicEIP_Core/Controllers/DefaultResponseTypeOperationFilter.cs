...
{
    /// <summary>
    /// 自訂的 OpenAPI 運行過濾器，用於根據方法上的 DefaultResponseTypeAttribute 屬性設定回應的預設 Schema 和狀態碼描述。
    /// </summary>
    public class DefaultResponseTypeOperationFilter : IOperationFilter
    {
        /// <summary>
        ...
        {
            // 獲取 DefaultResponseTypeAttribute 屬性
            var defaultResponseTypeAttribute = context.MethodInfo
                .GetCustomAttributes(typeof(DefaultResponseTypeAttribute), false)
                .FirstOrDefault() as DefaultResponseTypeAttribute;

            // 如果找到屬性則繼續處理
            if (defaultResponseTypeAttribute != null)
            {
                // 使用 SchemaGenerator 生成指定 ResponseType 的 Schema
                var schema = context.SchemaGenerator.GenerateSchema(
                    defaultResponseTypeAttribute.ResponseType,
