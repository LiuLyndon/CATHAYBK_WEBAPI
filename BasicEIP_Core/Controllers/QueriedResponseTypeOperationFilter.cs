...
{
    /// <summary>
    /// 自訂的 OpenAPI 運行過濾器，用於根據方法上的 QueriedResponseTypeAttribute 設置回應的 Schema。
    /// </summary>
    public class QueriedResponseTypeOperationFilter : IOperationFilter
    {
        /// <summary>
        ...
        {
            // 獲取 QueriedResponseTypeAttribute 屬性
            var queriedResponseTypeAttribute = context.MethodInfo
                .GetCustomAttributes(typeof(QueriedResponseTypeAttribute), false)
                .FirstOrDefault() as QueriedResponseTypeAttribute;

            // 如果找到屬性則繼續處理
            if (queriedResponseTypeAttribute != null)
            {
                // 使用 SchemaGenerator 生成指定 ResponseType 的 Schema
                var schema = context.SchemaGenerator.GenerateSchema(
                    queriedResponseTypeAttribute.ResponseType,
